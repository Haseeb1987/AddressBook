...
public class UpdateRec {
    ...
    public UpdateRec() {
     ...
     public void Update(){
        ...
        JButton adr=new JButton("Update Record");
        adr.setBounds(170,175,120,30);
        c.add(adr);
        adr.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {                
                try{
                    ...
                    st.executeUpdate("UPDATE AddressBook SET Name='"+Name+"',Fname='"+Fname+"',ID='"+ID+"',Phone='"+Phone+"',city='"+city+"',Gender='"+Gender+"' WHERE ID='"+ID+"'");
                    con.close();
                }
                    catch(Exception s){
                        System.out.print(s);
